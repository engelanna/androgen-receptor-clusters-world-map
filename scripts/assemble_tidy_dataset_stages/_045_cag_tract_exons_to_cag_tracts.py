...
from src.extracts import extract_full_cag_tract
...
def cag_tract_exons_to_cag_tracts(
    ...
    for input_fasta_file_name in sorted(_just_the_nonhidden_files):
        ...
        with open(input_fasta_file_path, "r") as input_fasta_file:
            ...
            if validate_exon_has_full_cag_tract(cag_exon):
                ...
                print(f"wrote {full_cag_tract} to {output_fasta_file_path}")
            else:
                print(f"dropping exon {cag_exon} because of incomplete CAG tract")
